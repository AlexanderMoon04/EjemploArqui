 ...
 // Updated by:      Josué Grajeda y Jaime Luna
 //Date:             04/2024
...
void HVAC_Enc_Apg_Ctrl (void)
{
    ...
    if(Enc_Apg == APAGADO){
        ...
        //Esto sucede tras pulsar por primera ocasión el botón de apagado/encendido tras el inicio del sistema
...
void HVAC_InicialiceADC(void)
{
    ...
    ADC_ConfigurePinChannel(POT1, AN8, ADC_VCC_VSS); //Se toma en cuenta el pin 4.3 para iniciar la lectura
...
void INT_UP_DOWN (void)
{
    ...
    if (((GPIO_getInputPinValue(UP_DOWN_PORT, BIT(UP_BTN)) != NORMAL_STATE_EXTRA_BUTTONS))
        ...
        switch (Select_Menu) {
            ...
            case P1_SELECTED:
                if(GPIO_getInputPinValue(UP_DOWN_PORT, BIT(UP_BTN)) == 0x01){ //Verifica si se pulsó el botón
                    UART_putsf(MAIN_UART,"\n\r Opening P1... \n\r"); //Manda mensaje por terminal
                    Persiana1.Estado = on; //Actualiza el estado
                }
                else if (GPIO_getInputPinValue(UP_DOWN_PORT, BIT(DOWN_BTN)) == 0x01){ //Verifica si se pulsó el botón
            ...
            case P2_SELECTED:
                if(GPIO_getInputPinValue(UP_DOWN_PORT, BIT(UP_BTN)) == 0x01){
                    UART_putsf(MAIN_UART,"\n\r Opening P2... \n\r");
                    Persiana2.Estado = on;
                }
                else if (GPIO_getInputPinValue(UP_DOWN_PORT, BIT(DOWN_BTN)) == 0x01){
